...
class StringPropertyStream extends PropertyStream<String> {
  ...
  late final Stream<String> _cachedStream =
      ReplayableBroadcastStream<String>(_replayableStreamFactory);
  ...
  Stream<String> get stream => _cachedStream;
}
...
class ListPropertyStream<T extends Object?> extends PropertyStream<List<T>>
  ...
  late final Stream<List<T>> _cachedStream =
      ReplayableBroadcastStream<List<T>>(_replayableStreamFactory);
  ...
  Stream<List<T>> get stream => _cachedStream;
...
class MapPropertyStream extends PropertyStream<Map<String, Object?>>
  ...
  late final Stream<Map<String, dynamic>> _cachedStream =
      ReplayableBroadcastStream<Map<String, dynamic>>(_replayableStreamFactory);
  ...
  /// Note: This stream will emit all past snapshots to new subscribers,
  /// preventing race condition issues when subscribing late.
  Stream<Map<String, dynamic>> get stream => _cachedStream;
...
/// A custom broadcast stream wrapper that creates and subscribes to a fresh
/// stream from a factory on every `listen` call, allowing late subscribers to
/// receive replayed buffer data.
class ReplayableBroadcastStream<T> extends Stream<T> {
  final Stream<T> Function() _factory;

  ReplayableBroadcastStream(this._factory);
